...
<body class="skin-green sidebar-mini">
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          ...
          <ol class="breadcrumb">
            ...
            <li class="active"><?php echo $lang['team']; ?> <?php echo $lang['list']; ?></li>
          </ol>
        </section>
                  ...
                  <table id="example1" class="table table-bordered table-striped">
                    <thead>
                      <tr>
					   ...
					   <th><?php echo $lang['facebook_url']; ?></th>
					   <th><?php echo $lang['twitter_url']; ?></th>
					   <th><?php echo $lang['linkedin_url']; ?></th>
                    ...
                    <tbody>
				    <?php
                     $query = DB::getInstance()->get("team", "*", ["userid" => $freelancer->data()->freelancerid, 'AND' => ["delete_remove" => 0]]);
                     if($query->count()) {
					    ...
					    echo '<td>
					      '.$mark.'
					      <a href="editteam.php?a=data&id=' . escape($row->teamid) . '" class="btn btn-success btn-xs" data-toggle="tooltip" title="' . $lang['edit'] . '"><span class="fa fa-edit"></span></a>
	...
	//Built a url to send
	var info = 'id=' + id;
	 if(confirm("<?php echo $lang['delete_teammate']; ?>"))
			  {
			var parent = $(this).parent().parent();
				$.ajax({
				 type: "GET",
				 url: "template/delete/deleteteammate.php",
				 data: info,
				 success: function()
					   {
	...
	//Built a url to send
	var info = 'id=' + id;
	 if(confirm("<?php echo $lang['activate_teammate']; ?>"))
			  {
			var parent = $(this).parent().parent();
				$.ajax({
				 type: "GET",
				 url: "template/actions/activateteammate.php",
				 data: info,
				 success: function()
					   {
	...
	//Built a url to send
	var info = 'id=' + id;
	 if(confirm("<?php echo $lang['deactivate_teammate']; ?>"))
			  {
			var parent = $(this).parent().parent();
				$.ajax({
				 type: "GET",
				 url: "template/actions/deactivateteammate.php",
				 data: info,
				 success: function()
					   {
						window.location.reload();
