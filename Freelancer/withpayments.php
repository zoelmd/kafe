		 ...
		 <div class="row">	
		 	...
		 	<div class="col-md-12">
                ...
                <div class="box-header">
                  <h3 class="box-title"><?php echo $lang['withdrawal']; ?> <?php echo $lang['payments']; ?> <?php echo $lang['list']; ?></h3>
                </div><!-- /.box-header -->
                <div class="box-body">
                  ...
                  <table id="example1" class="table table-bordered table-striped">
                    <thead>
                      <tr>
					   ...
					   <th><?php echo $lang['date']; ?> <?php echo $lang['paid']; ?></th>
                      </tr>
                    </thead>
                    <tbody>
				    <?php
						$query = DB::getInstance()->get("pay_freelancer", "*", ["AND" =>["freelancerid" => $freelancer->data()->freelancerid, "complete" => 1]]);
						if ($query->count()) {
						 foreach($query->results() as $row) {
									
								
							    $q1 = DB::getInstance()->get("freelancer", "*", ["freelancerid" => $row->freelancerid, "LIMIT" => 1]);
								if ($q1->count() === 1) {
								 foreach($q1->results() as $r1) {
								  $reporter .='
								             <img src="../Freelancer/'. escape($r1->imagelocation) .'" class="img-responsive img-thumbnail" width="50" height="40"/>
								             <a href="../freelancer.php?a=overview&id='. $freelancerid .'" target="_blank">'. escape($r1->name) .'</a>
								            ';	
								 }
								}else {
									$reporter .='';
							    ...
							    echo '<tr>';
					            echo '<td>'. $reporter .'</td>';
							    echo '<td>'. escape($currency_symbol) .' '. escape($row->amount_sum) .'</td>';
								
							    if (escape($row->complete) == 1) {
							    echo '<td><span class="label label-success"> ' . $lang['paid'] . ' </span> </td>';
								} else {
							    echo '<td><span class="label label-success"> ' . $lang['not'] . ' ' . $lang['paid'] . '</span> </td>';
								}
							    echo '<td>'. escape($row->month_time) .' </td>';
