...
<html lang="en-US" class="no-js">
                ...
                <div class="box-body">
                  <div class="table-responsive mailbox-messages">
				    ...
				    <?php
                     ...
                     $query = DB::getInstance()->get("message", "*", ["ORDER" => "date_added DESC", 'AND' => ["user_to" => $client->data()->clientid, "active" => 1, "delete_remove" => 0, "disc" => 0]]);
                     if($query->count()) {
 ...
 if ($q5->count() === 0) {
					    ...
					    $q1 = DB::getInstance()->get("freelancer", "*", ["freelancerid" => $row->user_from]);
						if ($q1->count()) {
						...
						if($row->opened == 0):						
					    echo '<tr class="success" id="comment'.$row->id.'">';
						else:				
					    echo '<tr id="comment'.$row->id.'">';
						endif;
					    echo '<td>
		                 <div id="message-action'.$row->id.'">
					      '.star($row->id, $clientid, null) .'
					     </div> 
					      </td>';
					    echo '<td class="mailbox-name"><a href="messagenotification.php?id=' . escape($row->user_from) . '">'. escape($name1) .'</a></td>';
					    echo '<td class="mailbox-subject">'. $blurb .'</td>';
					    echo '<td class="mailbox-date">'. escape(strftime("%b %d, %Y, %H : %M %p ", strtotime($row->date_added))) .'</td>';
						
					    echo '<td>
					      <a href="messagenotification.php?id=' . escape($row->user_from) . '" class="btn btn-primary btn-xs" data-toggle="tooltip" title="' . $lang['view'] . '"><span class="fa fa-eye"></span></a>
					      <a onclick="delete_not('.$row->id.', '.$clientid.')" class="btn btn-danger btn-xs" data-toggle="tooltip" title="' . $lang['delete'] . '"><i class="fa fa-trash-o"></i></a></td>';
