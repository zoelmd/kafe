...
<html lang="en-US" class="no-js">
 ...
 <body class="skin-green sidebar-mini">
                ...
                <div class="box-body">
                  <div class="table-responsive mailbox-messages">
				    ...
				    <?php
                     ...
                     $query = DB::getInstance()->get("trash", "*", ["ORDER" => "date_added DESC", 'AND' => ["userid" => $client->data()->clientid, "delete_remove" => 0, "disc" => 0]]);
                     if($query->count()) {
					    ...
					    $restore = '
					    <a onclick="restore_not('.$r1->id.', '.$clientid.')" class="btn btn-info btn-xs" data-toggle="tooltip" title="' . $lang['restore'] . '"><i class="fa fa-check"></i></a>';
						else:
					    echo '<td class="mailbox-name"><a href="message.php?id=' . escape($r1->user_to) . '">'. escape($name1) .'</a></td>';
					    $restore = '
					    <a onclick="restore_the('.$r1->id.', '.$clientid.')" class="btn btn-info btn-xs" data-toggle="tooltip" title="' . $lang['restore'] . '"><i class="fa fa-check"></i></a>';
						endif;	
	...
	function restore_the(id, clientid) {
	...
	function restore_not(id, clientid) {
		// id = unique id of the message/comment/chat
		// type = type of post: message/comment/chat
		$('#del_comment_'+id).html('<div class="preloader-retina"></div>');
