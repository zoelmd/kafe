      ...
      foreach ($names as $value) {
			...
			try{
			   $messageInsert = DB::getInstance()->insert('message', array(
				   ...
				   'user_from' => $client->data()->clientid,
				   'user_to' => $value,
				   'opened' => 0,
				   'active' => 1,
				   'delete_remove' => 0,
                  ...
                  <div class="form-group">	
					...
					<select name="client_name[]" type="text" class="form-control select" multiple="multiple">
					 <?php
					  ...
					  $query = DB::getInstance()->get("freelancer", "*", ["AND" => ["active" => 1, "delete_remove" => 0]]);
						if ($query->count()) {
						   $freelancername = '';
						   $x = 1;
							 foreach ($query->results() as $row) {
							  echo $freelancername .= '<option value = "' . $row->freelancerid . '">' . $row->name . '</option>';
							  unset($freelancername); 
							  $x++;
						     }
						}
					 ?>	
