		 ...
		 <div class="row">	
            ...
            <div class="col-md-9">
              <div class="box box-info">
                <div class="box-header with-border">
                  <h3 class="box-title"> <?php echo $lang['favorite']; ?> <?php echo $lang['mail']; ?></h3>
                </div><!-- /.box-header -->
                <div class="box-body">
                  <div class="table-responsive mailbox-messages">
				    ...
				    <?php
				     $clientid = $client->data()->clientid;
 					 $query = DB::getInstance()->get("star", ["[>]message" => ["star.messageid" => "id"]], "*", ["ORDER" => "star.date_added DESC", 'AND' => ["star.userid" => $client->data()->clientid, "message.delete_remove" => 0, "message.disc" => 0]]);
                     if($query->count()) {
						foreach($query->results() as $row) {
							
 $q5 = DB::getInstance()->get("trash", "*", ["AND" => ["messageid" => $row->id, "userid" => $clientid], "LIMIT" => 1]);
 if ($q5->count() === 0) {							
							
					    $q1 = DB::getInstance()->get("message", "*", ["AND" => ["id" => $row->messageid, "delete_remove" => 0], "LIMIT" => 1]);
						if ($q1->count()) {
							 foreach ($q1->results() as $r1) {	
						     }
						}
						
						$q2 = DB::getInstance()->get("client", "*", ["clientid" => $r1->user_from]);
						if ($q2->count() === 0) {
						    $q3 = DB::getInstance()->get("freelancer", "*", ["freelancerid" => $r1->user_from]);
					    ...
					    echo '<td>
		                 <div id="message-action'.$r1->id.'">
					      '.star($r1->id, $clientid, null) .'
					     </div> 
					      </td>';
					      
					    if ($clientid === $r1->user_to):
					    echo '<td class="mailbox-name"><a href="message.php?id=' . escape($r1->user_from) . '">'. escape($name1) .'</a></td>';
					    $mark = '
					    <a href="message.php?id=' . escape($r1->user_from) . '" class="btn btn-primary btn-xs" data-toggle="tooltip" title="' . $lang['view'] . '"><span class="fa fa-eye"></span></a>';
					    $delete = '
					    <a onclick="delete_not('.$r1->id.', '.$clientid.')" class="btn btn-danger btn-xs" data-toggle="tooltip" title="' . $lang['delete'] . '"><i class="fa fa-trash-o"></i></a>';
						else:
					    echo '<td class="mailbox-name"><a href="message.php?id=' . escape($r1->user_to) . '">'. escape($name1) .'</a></td>';
					    $mark = '
					    <a href="message.php?id=' . escape($r1->user_to) . '" class="btn btn-primary btn-xs" data-toggle="tooltip" title="' . $lang['view'] . '"><span class="fa fa-eye"></span></a>';
					    $delete = '
					    <a onclick="delete_the('.$r1->id.', '.$clientid.')" class="btn btn-danger btn-xs" data-toggle="tooltip" title="' . $lang['delete'] . '"><i class="fa fa-trash-o"></i></a>';
						endif;		
