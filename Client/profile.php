      ...
      if(in_array($ext,$valid_formats) && $size<(3024*3024))
	   {
         ...
         if(move_uploaded_file($tmp, $path.$actual_image_name))
	      {
	       if ($client->data()->imagelocation !== 'uploads/default.png') {
				unlink($client->data()->imagelocation);
...
if (Input::exists()) {
  if (Token::check(Input::get('token'))) {
    ...
    if (!$validation->fails()) {
		...
		if (Hash::make(Input::get('password_current'), $client->data()->salt) !== $client->data()->password) {
			$hasError = true;
		} else {
		  $salt = Hash::salt(32);
		  $client->update([
      ...
      if(in_array($ext,$valid_formats) && $size<(1920*1280))
	   {
         ...
         if(move_uploaded_file($tmp, $path.$actual_image_name))
	      {
	       if ($client->data()->bgimage !== 'uploads/bg/default.jpg') {
	       	$new_bgimage = $bgimage;
			unlink($new_bgimage);
				$siteUpdate = DB::getInstance()->update('client',[
				    'bgimage' => $newname
			    ],[
			    'clientid' => $client->data()->clientid
			    ]);
                  ...
                  <div class="box-body">
                    <div class="form-group">
					 <div class="image text-center">
					  ...
					  echo escape($client->data()->imagelocation); 
                  ...
                  <div class="box-body">
                    <div class="form-group">
					 <div class="image text-center">
					  ...
					  echo escape($client->data()->bgimage); 
